...
- [ ] Create compatibility views (map legacy names used in code/docs to canonical T4.4 tables)
  - `villages` -> `vdata`
  - `alliances` -> `alidata`
  - `marketplace` -> `market`
...
- [ ] Create `handshake` table in each world DB (required by FIX-03 login flow if not present)
  - SQL:
    ```sql
    USE travian_testworld;
...
- [ ] Re-run connectivity verification
